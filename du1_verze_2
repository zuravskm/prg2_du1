...
VIEW_URL = "Ukol1/du1.qml"   # Advanced user interface
...
class BrowserCityModel(QAbstractListModel):
    ...
    def load_from_json(self,filename):
        ...
        with open(filename,encoding="utf-8") as f:
            ...
            for c in self.city_list['features']:
    ...
    def data(self, index:QtCore.QModelIndex, role:int=...) -> typing.Any:
        ...
        if role == QtCore.Qt.DisplayRole:
            return self.city_list[index.row()]['features']['properties']['NAZ_OBEC']
        if role == self.Roles.POPULATION.value:
            return self.city_list[index.row()]['features']['properties']['POCET_OBYV']
        if role == self.Roles.AREA.value:
            return self.city_list[index.row()]['features']['properties']['SHAPE_Area']
        if role == self.Roles.LOCATION.value:
            return self.city_list[index.row()]['features']['geometry']['coordinates']
